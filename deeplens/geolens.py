...
class GeoLens(Lens):
    ...
    def prepare_sensor(self, sensor_res=None, sensor_size=None):
        ...
        print(f"Sensor size: {self.sensor_size}, Pixel size: {self.pixel_size} mm")
    ...
    def forward_tracing(self, ray, lens_range, record):
        ...
        mat1 = Material("air")
        for i in lens_range:
            if torch.is_tensor(ray.wvln):
                ray.wvln = ray.wvln.cpu().numpy()
    ...
    def pupil_field(self, point, wvln=DEFAULT_WAVE, spp=SPP_COHERENT):
        ...
        print("The point is at", point_obj)
        print("The incident angle is", torch.rad2deg(torch.arctan2(point_obj[0, 0], -point_obj[0, 2])))
    ...
    def draw_layout(
        ...
        # ==> Plot RGB in one figure
            ...
            fig.savefig(f"{filename}.png", format="png", dpi=600, transparent=True)
            plt.close()
    ...
    def optimize(
        ...
        for i in range(iterations + 1):
            ...
            for j, wv in enumerate(WAVE_RGB):
                ...
                weight_mask /= (weight_mask.mean() + EPSILON)  # shape of [M, M]
            ...
            # for j in range(1, len(self.surfaces)):
            #     print(self.surfaces[j].c, self.surfaces[j].d, self.surfaces[j].k, self.surfaces[j].ai)
            ...
            for j in range(1, len(self.surfaces)):
                assert not torch.isnan(self.surfaces[j].c)
                assert not torch.isnan(self.surfaces[j].d)
                assert not torch.isnan(self.surfaces[j].k)
    ...
    def read_lens_zmx(self, filename="./test.zmx"):
        ...
        for surf_idx, surf_dict in surfs_dict.items():
            ...
            if surf_idx > 0 and surf_idx < current_surf:
                ...
                # Image sensor
                surf_dict["DIAM"] = str(2076 * 2.4 * 1e-3) # force
